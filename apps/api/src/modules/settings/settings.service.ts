...
export class SettingsService {
   ...
   * Priority:
   *   1. TenantContext.getTenantId() — set by TenantMiddleware from JWT or x-tenant-id header
   *   2. DB fallback via userId — handles stale JWTs that have tenantId=null
   */
  private async resolveTenantId(): Promise<string> {
    const tenantId = TenantContext.getTenantId();
    if (tenantId) return tenantId;

    // Fallback: look up the user's tenantId from the DB using their userId
  ...
  async rules() {
    ...
    const activePlan = subscriptionSetting?.settingsJson && typeof subscriptionSetting.settingsJson === "object" && "activePlan" in subscriptionSetting.settingsJson
      ? (subscriptionSetting.settingsJson as any).activePlan
      : "Standard";
